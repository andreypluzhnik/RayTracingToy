...
scale::scale(shared_ptr<hittable> p, double s) : ptr(p), scaling(s) 
{
...
rotate_z::rotate_z(shared_ptr<hittable> p, double angle) : ptr(p) {
    ...
    for(int i  = 0; i < 2; i++){
        for(int j = 0; j < 2; j++){
            for(int k = 0; k < 2; k ++){
                test_vector[0] = (1 - i) * bbox.min()[0] + i * bbox.max()[0];
                test_vector[1] = (1 - j) * bbox.min()[1] + j * bbox.max()[1];
                test_vector[2] = (1 - k) * bbox.min()[2] + k * bbox.max()[2];

                test_vector[0] = test_vector[0] * cos_theta + test_vector[1] * sin_theta;
                test_vector[1] = test_vector[1] * cos_theta - test_vector[0] * sin_theta;
